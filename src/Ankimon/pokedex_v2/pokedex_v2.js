...
window.initializePokedex = async function(data) {
    console.log("Initializing Pokédex V2 with collection data...");
...
function applyInitialUIState() {
    ...
    const viewBtn = document.querySelector(`.view-btn[data-view="${state.ui.viewMode}"]`);
    if (viewBtn) {
    ...
    if (spriteToggleText) {
        spriteToggleText.textContent = state.ui.spriteMode === 'static' ? 'Static' : 'Animated';
    }
}

window.getPokedexState = function() {
    return {
...
async function loadSpeciesData() {
    try {
        const response = await fetch('../user_files/data_files/pokedex.json');
        ...
        rawList.forEach(p => {
            ...
            const isBase = !p.forme || ['base', 'normal', 'land', 'altered', '50%'].includes(p.forme.toLowerCase());
            if (!existing || isBase) {
                fullMap[id] = p;
            }
        ...
        state.allPokemon.sort((a, b) => {
            ...
            const isBaseA = a.actual_id === a.species_id || (a.forme && a.forme.toLowerCase() === 'base');
            const isBaseB = b.actual_id === b.species_id || (b.forme && b.forme.toLowerCase() === 'base');
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.nav-item').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (scope) {
                state.ui.currentScope = scope === 'national' ? 'national' : parseInt(scope);
                document.querySelectorAll('.nav-item[data-scope]').forEach(b => b.classList.remove('active'));
            }
            if (filter) {
    ...
    document.getElementById('sprite-toggle').addEventListener('click', () => {
        state.ui.spriteMode = state.ui.spriteMode === 'static' ? 'animated' : 'static';
    ...
    document.getElementById('clear-filters').addEventListener('click', () => {
        ...
        document.querySelector('.nav-item[data-scope="national"]').classList.add('active');
...
function setupZoomControls() {
    ...
    if (collapseBtn) collapseBtn.addEventListener('click', toggleCollapseCompleted);
...
function setupMapDragging() {
    ...
    canvas.addEventListener('mousedown', e => {
        if (e.target.closest('.discovery-node') || e.target.closest('.map-toolbar') || e.target.closest('.mini-map-container')) return;
...
function centerOnSelected() {
    ...
    const node = document.querySelector(`.discovery-node[data-id="${state.ui.selectedPokemonId}"]`);
    if (!node) return;
    const canvas = document.getElementById('map-canvas-area');
...
function applyMapFilter() {
    ...
    document.querySelectorAll('.discovery-node').forEach(node => {
        ...
        if (mapState.collapseCompleted && ns === 'caught' && filter !== 'completed') {
...
function renderSidebarFilters() {
    ...
    GEN_RANGES.forEach(gen => {
        ...
        const genPokemon = state.allPokemon.filter(p => p.species_id >= gen.start && p.species_id <= gen.end);
        ...
        state.collection.owned.forEach(id => {
            ...
            if (p && p.species_id >= gen.start && p.species_id <= gen.end) caughtInGen.add(p.species_id);
        });
        const caughtCount = caughtInGen.size;
        const progressPercent = totalInGen > 0 ? (caughtCount / totalInGen) * 100 : 0;
        btn.innerHTML = `
...
function updateGlobalProgress() {
    ...
    document.getElementById('global-progress-count').textContent = `${caughtCount} / ${total} Caught`;
    const seenEl = document.getElementById('global-seen-count');
    if (seenEl) seenEl.textContent = `(${seenCount} Seen)`;
}

function applyFiltersAndRender(forceMap = false) {
    const seenSpecies = new Set();
    let filtered = state.allPokemon.filter(p => {
        ...
        if (state.ui.activeFilter === 'shiny' && !state.collection.shinies.has(p.actual_id)) return false;
    ...
    const needsRender = (newVisibleIds.join(',') !== state.visibleIds.join(',')) || 
    ...
    if (state.ui.activeMode === 'collection') {
        ...
        document.getElementById('detail-panel').classList.remove('hidden');
        if (forceMap || state.mapDirty || document.getElementById('discovery-nodes').children.length === 0) {
            renderDiscoveryMap();
...
function updatePanelVisibility() {
    ...
    if (state.ui.selectedPokemonId) {
        ...
        // Show placeholder
        if (effectiveMode === 'details') detailPlaceholder.classList.remove('hidden');
...
function getDisplayId(p) {
    ...
    const isBase = p.actual_id === p.species_id || !p.forme || p.forme.toLowerCase() === 'base';
    return isBase ? p.species_id : p.actual_id;
}

function getSpritePath(id, mode = state.ui.spriteMode) {
    if (mode === 'animated') return `../user_files/sprites/front_default_gif/${id}.gif`;
...
function handleSpriteError(img, id, speciesId) {
    ...
    if (!img.src.endsWith('0.png')) img.src = '../user_files/sprites/front_default/0.png';
}
...
function renderGrid() {
    ...
    const isSpecialSort = ['name-asc', 'name-desc', 'stat-desc'].includes(state.ui.sortMode);
    const groups = [];
    ...
    groups.forEach(group => {
        ...
        group.ids.forEach(id => {
            ...
            if (state.collection.shinies.has(id)) card.querySelector('.shiny-badge').classList.remove('hidden');
...
function selectPokemon(id, cardElement = null) {
    ...
    if (visState === 0) sprite.style.filter = 'brightness(0) invert(1)';
    else if (visState === 1) sprite.style.filter = 'brightness(0) invert(1) brightness(0.5)';
    else sprite.style.filter = 'none';
    const primaryType = p.types[0].toLowerCase();
    document.getElementById('det-glow').style.background = `radial-gradient(circle, var(--type-${primaryType}) 0%, transparent 70%)`;
    document.getElementById('det-glow').style.opacity = visState >= 1 ? '0.2' : '0';
    ...
    document.getElementById('det-stat-total').textContent = visState === 2 ? total : '???';
    document.getElementById('det-height').textContent = visState === 2 ? `${p.heightm}m` : '???';
    document.getElementById('det-weight').textContent = visState === 2 ? `${p.weightkg}kg` : '???';
    const genInfo = GEN_RANGES.find(g => p.species_id >= g.start && p.species_id <= g.end);
    document.getElementById('det-gen').textContent = genInfo ? genInfo.name : 'Unknown';
    document.getElementById('det-category').textContent = visState === 2 ? (p.color || 'Unknown') : '???';
    renderForms(p);
    renderEvolution(p);
...
function renderBriefing(p, id, visState, displayId) {
    ...
    sprite.src = getSpritePath(id);
    sprite.onerror = () => handleSpriteError(sprite, id, p.species_id);
    if (visState === 0) sprite.style.filter = 'brightness(0) invert(1) brightness(0.2)';
    else if (visState === 1) sprite.style.filter = 'brightness(0) invert(1) brightness(0.5)';
    else sprite.style.filter = 'none';
    ...
    if (isOwned) {
        ...
        summaryEl.textContent = 'All prerequisites met. This target is unlocked and ready for encounter.';
        ...
        summaryEl.textContent = `${caughtCount} of ${totalReqs} prerequisites caught.`;
        
        const missingReqs = reqs.filter(rId => !state.collection.owned.has(rId));
        if (isOR) {
            nextStepEl.textContent = `Catch any one of the alternative prerequisites.`;
            ...
            nextStepEl.textContent = `Catch ${missingReqs.length} more prerequisites.`;
        }
    } else if (reqs.length > 0) {
        badgeEl.textContent = 'Locked';
        badgeEl.classList.add('locked');
        summaryEl.textContent = 'Prerequisites have not been met yet.';
        ...
        if (isOR) {
            nextStepEl.textContent = `Catch any one of the alternative prerequisites to unlock.`;
        } else {
        ...
        summaryEl.textContent = isEncounterable 
            ? 'No prerequisites required. This target is available.' 
    ...
    if (reqsMet && !isEncounterable && !isOwned && reqs.length > 0) {
         ...
         summaryEl.textContent = 'Prerequisites appear met, but target is restricted by level or other conditions.';
    ...
    document.getElementById('briefing-reqs-count').textContent = reqs.length > 0 ? `${caughtCount} / ${totalReqs}` : 'None';
    ...
    if (reqs.length > 0) {
        reqs.forEach(reqId => {
            ...
            let rFilter = '';
            if (rVisState === 0) rFilter = 'filter: brightness(0) invert(1) brightness(0.2)';
            else if (rVisState === 1) rFilter = 'filter: brightness(0) invert(1) brightness(0.5)';
        ...
        reqsListEl.innerHTML = '<div style="font-size:0.8rem; color:var(--text-muted);">No prerequisites required.</div>';
    }
    ...
    Object.entries(state.prerequisites).forEach(([tId, tReqs]) => {
        let rList = Array.isArray(tReqs) && tReqs[0] === 'OR' ? tReqs[1] : (Array.isArray(tReqs) ? tReqs : [tReqs]);
    ...
    if (leadsTo.length > 0) {
        ...
        text.textContent = `Prerequisite for ${leadsTo.length} other target(s).`;
        ...
        leadsTo.slice(0, 5).forEach(lId => {
            ...
            if (lVisState === 0) img.style.filter = 'brightness(0) invert(1) brightness(0.2)';
            else if (lVisState === 1) img.style.filter = 'brightness(0) invert(1) brightness(0.5)';
...
function renderForms(p) {
    ...
    forms.forEach(form => {
        ...
        let filter = '';
        if (visState === 0) filter = 'filter: brightness(0) invert(1) opacity(0.03)';
        else if (visState === 1) filter = 'filter: brightness(0) invert(1) opacity(0.15)';
...
function renderEvolution(p) {
    ...
    const findNode = (name) => {
        ...
        return state.allPokemon.find(x => x.name.toLowerCase().replace(/[^a-z0-9]/g, '') === norm);
    };
    while (root.prevo) {
        const prevo = findNode(root.prevo);
        if (prevo) root = prevo; else break;
    }
    const chain = [];
    const queue = [root];
    ...
    while (queue.length > 0) {
        ...
        chain.push(current);
        if (current.evos) current.evos.forEach(evoName => {
    ...
    chain.forEach(node => {
        ...
        let evoCondition = "";
        if (node.prevo) {
            if (node.evoLevel) evoCondition = `<span class="evo-condition">Lv. ${node.evoLevel}</span>`;
            else if (node.evoType) {
                let text = "Special";
                switch (node.evoType) {
                    case 'levelFriendship': text = "Friendship"; break;
                    case 'useItem': text = node.evoItem || "Item"; break;
                    case 'trade': text = "Trade"; break;
                    case 'levelMove': text = "Move"; break;
                    case 'levelHold': text = "Hold"; break;
                }
        ...
        evoImg.onerror = () => handleSpriteError(evoImg, node.actual_id, node.species_id);
    ...
    if (chain.length <= 1) evoContainer.innerHTML = '<p style="font-size: 0.8rem; color: var(--text-muted)">No evolution chain</p>';
}

function toggleDrawer(id, cardElement) {
    if (document.querySelector('.expansion-drawer') && cardElement.classList.contains('has-drawer')) {
        closeDrawer();
        return;
    }
    closeDrawer();
    ...
    forms.forEach(form => {
        ...
        let filter = '';
        if (visState === 0) filter = 'filter: brightness(0) invert(1) brightness(0.2)';
        else if (visState === 1) filter = 'filter: brightness(0) invert(1) brightness(0.5)';
        ...
        chip.onclick = (e) => {
            ...
            document.querySelectorAll('.drawer-forms-list .form-chip').forEach(c => c.classList.remove('active'));
    ...
    const lastIndexInRow = Math.min(cards.length - 1, (rowIndex + 1) * columns - 1);
...
function renderAbilities(p, visState) {
    ...
    container.innerHTML = "";
    if (visState === 0) { section.classList.add("hidden"); return; }
    ...
    Object.entries(p.abilities).forEach(([key, name]) => {
        ...
        const description = state.abilities[abilityId] || "Detailed research data unavailable.";
...
function closeDrawer() {
    ...
    if (d) d.remove();
    document.querySelectorAll('.pokemon-card.has-drawer').forEach(c => c.classList.remove('has-drawer'));
}
...
function renderPrerequisites(p) {
    ...
    if (isMegaGmax && isActuallyForm) {
        ...
        reqs = state.prerequisites[p.species_id] || state.prerequisites[p.actual_id];
    }
...
const TARGET_IDS  = new Set([150, 249, 250, 384, 487, 490, 486, 645, 646, 647, 718, 773, 896, 897]); 
const LEGENDARY_PREREQUISITE_IDS = new Set([483,484,243,244,245,144,145,146,377,378,379,894,895,641,642,640,638,639,716,717,791,792,888,889,1014,1015,1016,489,898]); 

function buildMapNodes() {
    const nodes = {}; 
    const allIds = new Set();

    const legendaryTargetKeys = [150,249,250,384,487,490,486,645,646,647,718,773,800,890,896,897,905,1024,1025,493];
    ...
    while (stack.length > 0) {
        ...
        if (reqs) {
            const ids = Array.isArray(reqs) && reqs[0] === 'OR' ? reqs[1] : (Array.isArray(reqs) ? reqs : [reqs]);
            ids.forEach(r => { if (typeof r === 'number') stack.push(r); });
        }
    }
    ...
    Object.values(state.prerequisites).forEach(r => {
        const ids = Array.isArray(r) && r[0] === 'OR' ? r[1] : (Array.isArray(r) ? r : [r]);
        ids.forEach(id => { if (typeof id === 'number') allPrereqIds.add(id); });
    });
    const apexKeys = legendaryTargetKeys.filter(id => !allPrereqIds.has(id) || APEX_IDS.has(id));
    ...
    allIds.forEach(id => {
        ...
        const gk = targets.length > 0 ? targets.join('-') : (legendaryTargetKeys.includes(id) ? id.toString() : 'independent');
    ...
    sortedGroupKeys.forEach(gk => {
        ...
        const maxNodesInGroupTier = Math.max(1, ...groupNodesByTier.map(arr => arr.length));
    ...
    sortedGroupKeys.forEach(gk => {
        ...
        const groupHeight = Math.max(1, ...groupNodesByTier.map(arr => arr.length)) * NODE_SPACING;
        ...
        for (let t = groupNodesByTier.length - 1; t >= 0; t--) {
            ...
            tierIds.forEach((id, i) => {
                ...
                Object.entries(state.prerequisites).forEach(([childId, reqs]) => {
                    const rIds = Array.isArray(reqs) && reqs[0] === 'OR' ? reqs[1] : (Array.isArray(reqs) ? reqs : [reqs]);
                    if (rIds.includes(id) && nodes[childId] && nodes[childId].y !== null) children.push(nodes[childId]);
                });

                if (children.length > 0) {
                    nodes[id].y = children.reduce((sum, c) => sum + c.y, 0) / children.length;
                } else {
...
function renderDiscoveryMap() {
    ...
    Object.entries(state.prerequisites).forEach(([tId, reqs]) => {
        const targetId = parseInt(tId);
        if (!nodes[targetId]) return;
        let srcIds = Array.isArray(reqs) && reqs[0] === 'OR' ? reqs[1] : (Array.isArray(reqs) ? reqs : [reqs]);
    ...
    Object.entries(nodes).forEach(([idStr, pos]) => {
        ...
        let spriteFilter = '';
        if (visState === 0) spriteFilter = 'brightness(0) invert(1) brightness(0.2)'; 
        else if (visState === 1) spriteFilter = 'brightness(0) invert(1) brightness(0.5)';
        ...
        el.addEventListener('click', e => {
            e.stopPropagation();
            // Deselect all
            document.querySelectorAll('.discovery-node.selected').forEach(n => n.classList.remove('selected'));
...
function updateMapProgress(nodes, chains, finalTargets) {
    ...
    visualChains.forEach(gk => {
        const groupIds = chains[gk].ids;
        const isDone = groupIds.some(id => (APEX_IDS.has(id) || TARGET_IDS.has(id)) && state.collection.owned.has(id));
        if (isDone) completedChains++;
    });


    const percent = totalNodes > 0 ? Math.round((caughtNodes / totalNodes) * 100) : 0;
    const chainsPercent = totalChains > 0 ? Math.round((completedChains / totalChains) * 100) : 0;
    ...
    if (chainFillEl) chainFillEl.style.width = `${chainsPercent}%`;
    if (chainsEl) chainsEl.textContent = `${completedChains} / ${totalChains} Chains`;
}
...
function getNodeState(id) {
    ...
    if (Array.isArray(reqs) && reqs[0] === 'OR') {
        const any = reqs[1].some(rid => typeof rid === 'number' && state.collection.owned.has(rid));
        return any ? 'available' : 'locked';
    } else {
        const ids = Array.isArray(reqs) ? reqs : [reqs];
        ids.forEach(rid => { if (typeof rid === 'number') { total++; if (state.collection.owned.has(rid)) metCount++; } });
