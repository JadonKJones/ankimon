...
class PokedexV2(QDialog):
    ...
    def get_pokedex_v2_data(self):
        """Fetches comprehensive collection data for V2."""
        ...
        return {
            ...
            "prefs": {
                "viewMode": mw.settings_obj.get("pokedex_v2.viewMode", "grid"),
                "sortMode": mw.settings_obj.get("pokedex_v2.sortMode", "id-asc"),
                "spriteMode": mw.settings_obj.get("pokedex_v2.spriteMode", "static"),
            },
            "evolutionNote": "In Ankimon, evolutions are currently supported through level progression only."
        }

    def load_initial_html(self):
        file_path = os.path.join(self.addon_dir, "pokedex_v2", "pokedex_v2.html").replace("\\", "/")
        url = QUrl.fromLocalFile(file_path)
        
        # We don't pass everything via URL query for V2 to avoid long URLs
        # Instead, we'll push data via JS right after load or on show
        self.webview.setUrl(url)

    def update_ui_data(self):
        """Pushes data to the frontend."""
        data = self.get_pokedex_v2_data()
        data_js = json.dumps(data)
        
        js_code = f"if (window.initializePokedex) window.initializePokedex({data_js});"
        ...
        def on_state_ready(state):
            if state and isinstance(state, dict):
                for key, val in state.items():
                    mw.settings_obj.set(f"pokedex_v2.{key}", val)

        if self.webview:
            self.webview.page().runJavaScript("if (window.getPokedexState) window.getPokedexState();", on_state_ready)
