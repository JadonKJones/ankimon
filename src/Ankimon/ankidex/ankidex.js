...
function updateGlobalProgress() {
    ...
    if (percent === lastGlobalPct && caughtCount === lastGlobalCaught && seenCount === lastGlobalSeen) return;
...
function applyFiltersAndRender(forceAll = false) {
    ...
    if (state.ui.activeMode === 'collection') {
        ...
        document.getElementById('detail-panel').classList.remove('hidden');
        if (forceAll || state.mapDirty || document.getElementById('discovery-nodes').children.length === 0) {
            renderDiscoveryMap();
...
function selectPokemon(id, cardElement = null) {
    ...
    if (visState === 0) sprite.style.filter = 'brightness(0) invert(1) brightness(0.15)';
    else if (visState === 1) sprite.style.filter = 'brightness(0) invert(1) brightness(0.4)';
...
function renderBriefing(p, id, visState, displayId) {
    ...
    if (reqsMet && !isEncounterable && !isOwned && reqs.length > 0) {
         ...
         summaryEl.textContent = 'Requirements appear met, but target is restricted by level or other conditions.';
    ...
    if (reqs.length > 0) {
        reqs.forEach(reqId => {
            ...
            let rFilter = '';
            if (rVisState === 0) rFilter = 'filter: brightness(0) invert(1) brightness(0.15)';
            else if (rVisState === 1) rFilter = 'filter: brightness(0) invert(1) brightness(0.4)';
        ...
        reqsListEl.innerHTML = '<div style="font-size:0.8rem; color:var(--text-muted);">No requirements.</div>';
    }
    ...
    if (leadsTo.length > 0) {
        ...
        leadsTo.slice(0, 5).forEach(lId => {
            ...
            if (lVisState === 0) img.style.filter = 'brightness(0) invert(1) brightness(0.15)';
            else if (lVisState === 1) img.style.filter = 'brightness(0) invert(1) brightness(0.4)';
        ...
        terminal.textContent = 'End of Chain: This target does not unlock further encounters.';
...
function toggleDrawer(id, cardElement) {
    ...
    forms.forEach(form => {
        ...
        let filter = '';
        if (visState === 0) filter = 'filter: brightness(0) invert(1) brightness(0.15)';
        else if (visState === 1) filter = 'filter: brightness(0) invert(1) brightness(0.4)';
...
function renderDiscoveryMap() {
    ...
    Object.entries(nodes).forEach(([idStr, pos]) => {
        ...
        let spriteFilter = '';
        if (visState === 0) spriteFilter = 'brightness(0) invert(1) brightness(0.15)'; 
        else if (visState === 1) spriteFilter = 'brightness(0) invert(1) brightness(0.4)';
...
function updateMapProgress(nodes, chains, finalTargets) {
    ...
    const dataChanged = (percent !== lastMapPct || chainsPercent !== lastChainPct || caughtNodes !== lastMapCaught);
    ...
    if (chainFillEl) {
        if (chainFillEl.getAttribute('data-last-pct') !== chainsPercent.toString()) {
