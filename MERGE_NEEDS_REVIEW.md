...
## NR-24 — Final integration review (Sonnet-5 fleet) + fixes  ·  OPEN (informational)
Context: after all feature waves merged (tip `fc4c15fa`), an 11-lane Sonnet-5 max-effort review swept the full `origin/main...origin/integration` diff (162 files). It surfaced **53 findings** (12 high / 26 medium / 15 low); an Opus fix workflow (5 disjoint file-partitions, each adversarially verified + a Fable batch check) resolved them onto tip `faf67f97` (gate 460 passed / 0 failed, +47 regression tests). **All 12 HIGH findings are fixed or honestly-deferred.**

Notable HIGH fixes (see PRs #576–#580): battle_loop `evo_window=None` no longer aborts `save_main_pokemon_progress` (silent battle-persistence loss); `check_evolution_by_item` trade-with-held-item restored (16 species were unevolvable); learnset `S`-code no longer leaks event moves into level-up movesets; Deck-Browser nickname **stored-XSS** escaped (vector was the F48 trade payload); `AnimatedStatBar` `QPropertyAnimation` parented (SIGABRT-class crash); mobile `record_desktop_review` wired (desktop reviews were double-processed); mobile **XP-Share** applied for desktop parity; `_parse_cards_per_round` ZeroDivision clamped; `on_sync_did_finish` dual-DB switch serialized under `_mobile_sync_lock`; SQLite `busy_timeout` added for mobile bg writes; `is_dev_mode()` helper added (F36 dev submenu was inert).

**Deferred (need changes beyond a single fixer's partition, or are future gameplay work):**
- **Desktop↔mobile companion read-modify-write lock (mobile_sync.py:1748):** the mobile side serializes under `_mobile_sync_lock`; closing the gap fully requires the desktop `kill_pokemon` / `xp_share_gain_exp` read-modify-write (in `encounter_functions.py` / `trainer_functions.py`) to take the same lock. Low-probability race; needs a cross-file change.
- **`utils.in_bulk_resolve` thread-scoping (mobile_sync.py:1897):** a background bulk-resolve can transiently suppress a concurrent *desktop* review's level-up/evolution UI. Correct fix converts the flag's storage (`utils.py`) + readers (`encounter_functions`, `pokedex_functions`) to `threading.local`.
- **F49 web-HUD live-refresh (trainer_card.py):** `events.emit('stats_changed')` is a drain-only harness-observability signal with no subscriber; the web HUD re-renders via `refresh_live_screen()` from shell-handle holders. To make trainer XP gains live-update the web Profile, either give the HOST an event subscription or call `notify_stats_changed()` through a resolved shell handle. The in-code comment is honest (does not claim it works).
- **Team "Active Companion" (team.js):** was dead end-to-end (hardcoded `''`, undefined picker helpers). The vestigial JS was **removed** so nothing claims a non-working feature; wiring it is future gameplay work (`services.db.set_main_pokemon` exists and is ready).
- **WAL (database_manager.py):** kept opt-in per NR-05 (single-file persistence guard); a 30s `busy_timeout` mitigates bulk-resolve lock contention. Enabling WAL for the mobile path requires the checkpoint-before-copy backup fix.
- **Profile dex-seen vs Ankidex count:** `get_all_pokemon_ids()` does not merge released-history (a deferred DB leaf); comment corrected so the Profile tile no longer overclaims parity with Ankidex.
