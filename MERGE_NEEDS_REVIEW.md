...
- **Mobile → XP-Share design divergence (substantive, Wave-5):** mobile replay XP is awarded via `save_main_pokemon_progress` and never routes through `kill_pokemon`'s `xp_share_gain_exp` (`pyobj/ankimon_sync.py`), so an XP-Share designee gains NO XP from mobile-synced battles. Not a dropped guard (None-checks intact) — a behavioral gap vs desktop. Follow-up: decide whether mobile battles should credit the XP-Share target.
