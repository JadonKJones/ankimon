...
# ------------------------------------------------------------------------------
# HEAVY MONTE-CARLO SIMULATION — opt-in only.
# This module is a report-generating script (run it directly:
#   python tests/encounter_weighting_simulations/test_encounter_simulation.py
# ). Importing it has module-level side effects (sys.modules mocks, sys.path
# inserts, pokedex.json load) and the scenarios run up to 500k generations, so
# the default `pytest tests/` run must not collect it. Set
# ANKIMON_RUN_SIMULATIONS=1 to allow collection under pytest.
# Report/JSON outputs land next to this file and are gitignored.
# ------------------------------------------------------------------------------
if __name__ != "__main__" and not os.environ.get("ANKIMON_RUN_SIMULATIONS"):
    import pytest

    pytest.skip(
        "Heavy Monte-Carlo encounter simulation (opt in with "
        "ANKIMON_RUN_SIMULATIONS=1 or run this file directly)",
...
modules_to_mock = [
    ...
    "anki",
    "anki.hooks",
    "addon.pyobj.ankimon_tracker",
    "addon.pyobj.pokemon_obj",
    "addon.pyobj.reviewer_obj",
    "addon.pyobj.test_window",
    "addon.pyobj.trainer_card",
    "addon.pyobj.InfoLogger",
    "addon.pyobj.evolution_window",
    "addon.pyobj.attack_dialog",
    "addon.pyobj.translator",
    "addon.pyobj.error_handler",
    "addon.functions.pokemon_functions",
    "addon.functions.pokedex_functions",
    "addon.functions.trainer_functions",
    "addon.functions.badges_functions",
    "addon.functions.drawing_utils",
    "addon.utils",
    "addon.business",
    "addon.const",
    "addon.singletons",
    "addon.functions.encounter_data",
    "addon.functions.friendship_evolution",
]
...
sys.modules["addon.pyobj.error_handler"].show_warning_with_traceback = (
    lambda *args, **kwargs: None
)

sys.modules["addon.functions.pokemon_functions"].find_experience_for_level = (
    lambda *args: 0
)
sys.modules["addon.functions.pokemon_functions"].get_levelup_move_for_pokemon = (
    lambda *args: None
)
sys.modules["addon.functions.pokemon_functions"].pick_random_gender = lambda *args: (
    "Male"
)
...
sys.modules["addon.functions.pokedex_functions"].check_evolution_for_pokemon = (
    lambda *args: None
)
...
sys.modules["addon.functions.pokedex_functions"].get_growth_rate = lambda *args: (
    "Medium Fast"
)
sys.modules[
    "addon.functions.pokedex_functions"
].return_name_for_id = search_pokedex_by_id
sys.modules["addon.functions.pokedex_functions"].search_pokedex = search_pokedex
sys.modules[
    "addon.functions.pokedex_functions"
].search_pokedex_by_id = search_pokedex_by_id
sys.modules["addon.functions.pokedex_functions"].safe_int = safe_int
sys.modules["addon.functions.pokedex_functions"].get_pretty_name_for_name = (
    lambda name: name
)
...
# encounter_functions now sits on main's service seam (`from ..services import
# services` / `from ..events import events`): load the real (aqt-free) modules
# under the private "addon" namespace so those relative imports resolve.
def _load_real_module(name, filepath):
...
# Dependency order: services.py pulls in .ui_port, which pulls in .events.
_load_real_module("addon.events", os.path.join(base_dir, "src", "Ankimon", "events.py"))
_load_real_module(
    "addon.ui_port", os.path.join(base_dir, "src", "Ankimon", "ui_port.py")
)
_load_real_module(
    "addon.services", os.path.join(base_dir, "src", "Ankimon", "services.py")
)

spec = importlib.util.spec_from_file_location(
    "addon.functions.encounter_functions",
    os.path.join(base_dir, "src", "Ankimon", "functions", "encounter_functions.py"),
)
ef = importlib.util.module_from_spec(spec)
sys.modules["addon.functions.encounter_functions"] = ef
spec.loader.exec_module(ef)

# The seam module initializes its bare globals (bind_runtime_globals targets)
# to None at exec; rebind them to the simulation mocks.
ef.main_pokemon = sys.modules["addon.singletons"].main_pokemon
ef.ankimon_tracker_obj = sys.modules["addon.singletons"].ankimon_tracker_obj
ef.trainer_card = sys.modules["addon.singletons"].trainer_card
ef.settings_obj = mock_settings
ef.translator = sys.modules["addon.singletons"].translator
ef.ankimon_db = sys.modules["addon.singletons"].ankimon_db
ef.pokemon_pc = sys.modules["addon.singletons"].pokemon_pc
...
ef.get_all_pokemon_in_tier = lambda tier: (
    ed.STARTERS if tier == "Starter" else original_get_all_pokemon_in_tier(tier)
)
...
def run_simulation(config, N=200000, force_tier=None):
    ...
    for count in range(N):
        ...
        res = ef.generate_random_pokemon(
            MAIN_POKEMON_LEVEL, sys.modules["addon.singletons"].ankimon_tracker_obj
        )
        name, pokemon_id, tier = res[0], res[1], res[14]

        forme = search_pokedex(name, "forme")
        generation = ef.get_base_species_gen(pokemon_id)
        is_regional = pokemon_id >= 10000 and forme in [
            "Alola",
            "Galar",
            "Hisui",
            "Paldea",
...
def run_all():
    ...
    print("====================================================")
    print(
        f"SETTINGS: Level={MAIN_POKEMON_LEVEL}, BypassLvl={BYPASS_MIN_LEVEL_CHECK}, BypassPre={BYPASS_PREREQUISITES}, BypassOwn={BYPASS_BASE_FORM_CHECK}"
    )
    print("----------------------------------------------------")
    ...
    if should_run(1) or should_run(4) or should_run(5) or should_run(6):
        ...
        if stats1["regional_count"] == 0:
            ...
            msgs1.append(
                f"PASS: Regional forms appear ({stats1['regional_count']}/{stats1['total']})"
            )
        ...
        meowth_galar = 10161
        m_total = (
            get_count(stats1, meowth_base)
            + get_count(stats1, meowth_alola)
            + get_count(stats1, meowth_galar)
        )
        if m_total > 0:
            ...
            msgs1.append(
                f"Meowth: Base {br:.1%}, Alola {ar:.1%}, Galar {gr:.1%} (target ~7% each)"
            )
    ...
    if should_run(2):
        ...
        if stats2["regional_count"] > 0:
            passed2 = False
            msgs2.append(
                f"FAIL: Regional forms appeared when gens 7/8/9 disabled ({stats2['regional_count']})"
            )
        else:
    ...
    if should_run(3):
        ...
        if stats3["by_forme"].get("Alola", 0) == 0:
            ...
            msgs3.append(
                f"PASS: Alolan forms appeared ({stats3['by_forme'].get('Alola')})"
            )
    ...
    if should_run(4):
        ...
        if gen1_s4 <= gen1_s1 + 0.05:
            msgs4.append(
                f"FAIL: Gen 1 rate in Scenario 4 ({gen1_s4:.1%}) not meaningfully higher than Scenario 1 ({gen1_s1:.1%})"
            )
            passed4 = False
        else:
            msgs4.append(
                f"PASS: Gen 1 rate increased from {gen1_s1:.1%} to {gen1_s4:.1%}"
            )
    ...
    if should_run(5):
        ...
        if valola_s5 <= valola_s1:
            msgs5.append(
                f"FAIL: Alolan Vulpix rate didn't increase ({valola_s1} -> {valola_s5})"
            )
            passed5 = False
        else:
        ...
        if alola_m <= galar_m:
            msgs5.append(
                f"FAIL: Alolan Meowth ({alola_m}) not strictly > Galarian Meowth ({galar_m})"
            )
            passed5 = False
        else:
            msgs5.append(
                f"PASS: Alolan Meowth ({alola_m}) > Galarian Meowth ({galar_m})"
            )

        if alola_m + base_m > 0:
            ratio = alola_m / (alola_m + base_m)
            msgs5.append(
                f"Alolan Meowth Ratio: {alola_m}/{alola_m + base_m} = {ratio:.1%} (predicted ~84.1%)"
            )
        ...
        if galar_m != 0:
            msgs5.append(
                f"FAIL: Galarian Meowth appeared when region is set to Alola (count: {galar_m})"
            )
            passed5 = False
        else:
            msgs5.append(
                "PASS: Galarian Meowth correctly has 0% appearance rate in Alola region"
            )
        ...
        if pg_count != 0:
            msgs5.append(
                f"FAIL: Galarian Ponyta appeared when region is set to Alola (count: {pg_count})"
            )
            passed5 = False
        else:
            msgs5.append(
                "PASS: Galarian Ponyta correctly has 0% appearance rate in Alola region"
            )

        if wp_count != 0:
            msgs5.append(
                f"FAIL: Paldean Wooper appeared when region is set to Alola (count: {wp_count})"
            )
            passed5 = False
        else:
            msgs5.append(
                "PASS: Paldean Wooper correctly has 0% appearance rate in Alola region"
            )
    ...
    if should_run(6):
        ...
        if hisui_s6 <= hisui_s1:
            passed6 = False
            msgs6.append("FAIL: Hisui forms not boosted")
    ...
    if should_run(12):
        ...
        if stats12["by_generation"].get(1, 0) == 0:
            ...
            msgs12.append(
                f"PASS: Gen 1 Megas appeared ({stats12['by_generation'].get(1, 0)} times)"
            )
    ...
    if should_run(14):
        ...
        if stats14["by_forme"].get("Galar", 0) == 0:
            ...
            msgs14.append(
                f"PASS: Galarian legendaries appeared ({stats14['by_forme'].get('Galar')} times)"
            )
    ...
    if should_run(17):
        ...
        ef.generate_random_pokemon(
            100, sys.modules["addon.singletons"].ankimon_tracker_obj
        )  # Warm up

        cfg_k = dict(base_config)
        cfg_k["misc.active_region"] = "kanto"
        encs_k = run_simulation(cfg_k, N=N[17])

        cfg_a = dict(base_config)
        cfg_a["misc.active_region"] = "alola"
        ...
        if k_in_a > a_in_a:
            passed17 = False
            msgs17.append(
                f"FAIL: Kanto rate ({k_in_a:.1%}) > Alola rate ({a_in_a:.1%}) in Alola session"
            )
        else:
    ...
    if should_run(18):
        ...
        if ma_count != 0 or mg_count != 0:
            passed18 = False
            msgs18.append(
                f"FAIL: Non-Paldean regional forms appeared when region is set to Paldea (Alola Meowth: {ma_count}, Galar Meowth: {mg_count})"
            )
        else:
            msgs18.append(
                "PASS: Non-Paldean regional forms correctly have 0% appearance rate in Paldea region"
            )
    ...
    if should_run(19):
        ...
        if t_p_count > 0:
            passed19 = False
            msgs19.append(
                f"FAIL: Paldean Tauros appeared despite Gen 1 disabled ({t_p_count} times)"
            )
        else:
    ...
    if should_run(20):
        ...
        m_total = (
            get_count(stats20, m_base)
            + get_count(stats20, m_alola)
            + get_count(stats20, m_galar)
        )
        if m_total > 0:
        ...
        t_total = get_count(stats20, t_base) + sum(
            get_count(stats20, tid) for tid in p_tauros
        )
        if t_total > 0:
