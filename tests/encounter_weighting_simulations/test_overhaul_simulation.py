...
# Importing this module has module-level side effects (sys.modules mocks) and
# test_simulation_suite() runs 4 x 5000 weighted rolls, so the default
# `pytest tests/` run must not collect it. Set ANKIMON_RUN_SIMULATIONS=1 to
# opt in, or run this file directly:
#   python tests/encounter_weighting_simulations/test_overhaul_simulation.py
# ------------------------------------------------------------------------------
if __name__ != "__main__" and not os.environ.get("ANKIMON_RUN_SIMULATIONS"):
    import pytest

    pytest.skip(
        "Heavy Monte-Carlo overhaul simulation (opt in with "
...
sys.modules["Ankimon.pyobj.error_handler"].show_warning_with_traceback = (
    lambda *args, **kwargs: None
)
sys.modules["Ankimon.functions.pokemon_functions"].pick_random_gender = lambda *args: (
    "Female"
)
...
sys.modules["Ankimon.functions.pokedex_functions"]._load_pokedex_cache = lambda: (
    pokedex_data
)
...
# Package stubs so relative imports resolve. Under pytest the repo conftest
# already provides these; when this file is run directly as a script it must
# create them itself (before encounter_functions is exec'd below).
import types

for _pkg in ("Ankimon", "Ankimon.functions", "Ankimon.pyobj"):
    if _pkg not in sys.modules:
        _pkg_mod = types.ModuleType(_pkg)
        _pkg_mod.__path__ = [str(_src / _pkg.replace(".", "/"))]
...
# so those relative imports resolve, and route the mock DB through services.db
# (the seam replacement for exp's `mw.ankimon_db`).
...
_load_real_module("Ankimon.events", str(_src / "Ankimon" / "events.py"))
_load_real_module("Ankimon.ui_port", str(_src / "Ankimon" / "ui_port.py"))
_services_mod = _load_real_module(
    "Ankimon.services", str(_src / "Ankimon" / "services.py")
)
_services_mod.services.db = mock_db
...
ef.main_pokemon = mock_pokemon
ef.trainer_card = mock_trainer
ef.settings_obj = mock_settings
ef.ankimon_tracker_obj = sys.modules["Ankimon.singletons"].ankimon_tracker_obj
...
def simulate_scenario_a():
    ...
    mock_db.user_data["ankimon_pity_trackers"] = {
        k: 0 for k in ef.OVERHAUL_PITY_THRESHOLDS
    }

    # Calculate EP mathematically
    ep = ef.calculate_mastery_index_ep(
        total_reviews=0, daily_average=100, trainer_level=1
    )
...
def simulate_scenario_b():
    ...
    mock_pokemon.level = (
        35  # locks Legendary, Mega, Gmax, Mythical, Starter; unlocks Ultra
    )
    ...
    pokedex_data.clear()
    pokedex_data.update(
        {str(i): {"species_id": i, "actual_id": i} for i in range(1, 101)}
    )

    # Top 6 pokemon each with 1200 CP -> Core team CP = 1200
    mock_db.pokemon_list = [
        {"level": 30, "stats": {}, "individual_id": str(i)} for i in range(6)
    ]
    ...
    ep = ef.calculate_mastery_index_ep(
        total_reviews=60, daily_average=100, trainer_level=20
    )
    ...
    for tier, count in counts.items():
        ...
        if tier in ["Legendary", "Mega", "Gmax", "Mythical", "Starter"]:
            if count > 0:
                passed = False
                errors.append(
                    f"Locked tier '{tier}' spawned under level lock thresholds!"
                )
...
def simulate_scenario_c():
    ...
    mock_db.pokemon_list = [
        {"level": 80, "stats": {}, "individual_id": str(i)} for i in range(6)
    ]
    ...
    log(
        f"Aggregate Rare Spawn Rate: {aggregate_rare_rate:.2f}% (Expected under 13% Soft Landing cap)"
    )

    if aggregate_rare_rate > 13.0:
        passed = False
        errors.append(
            f"Aggregate rare spawn economy inflated! Got {aggregate_rare_rate:.2f}% (Max allowed: 13%)"
        )
    elif aggregate_rare_rate < 1.0:
        passed = False
        errors.append(
            "Aggregate rare spawn rate heavily throttled below baseline expectations!"
        )
...
def simulate_scenario_d():
    ...
    if legendary_rate < 1.0:
        passed = False
        errors.append(
            f"Independent pity multiplier failed to boost Legendary spawn rate! Rate: {legendary_rate:.2f}%"
        )
    else:
        log(
            "PASS: Independent pity counter successfully boosted Legendary spawn rates!"
        )
    ...
    res = ef.generate_random_pokemon(
        85, sys.modules["Ankimon.singletons"].ankimon_tracker_obj
    )
    log(
        f"DEBUG: generate_random_pokemon returned name={res[0]}, id={res[1]}, tier={res[14]}"
    )
    ...
    if updated_pity["Legendary"] != 0:
        passed = False
        errors.append(
            "Legendary pity counter failed to reset to 0 after Legendary spawn!"
        )
    else:
...
def teardown_module():
    # Detach the mock DB from the (real) services module before restoring.
    _services_mod.services.db = None
    touched = modules_to_mock + [
        "Ankimon.functions.encounter_functions",
        "Ankimon.services",
        "Ankimon.ui_port",
        "Ankimon.events",
