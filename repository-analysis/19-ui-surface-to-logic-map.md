...
* **Mobile Reviews (`mobile.html` & `history.html`):** Renders State 1 (no reviews pending), State 2 (landing page with pending counts, ease breakdown, and roster selector), and State 3 (post-resolve summary showing XP, encounters, cash, trainer XP, and a detailed caught list). Supports **Auto-Resolve** (optimal team matchup selection based on moves, power, and speed matchup simulation — runs in a daemon background thread) and **Manual Replay** (step-by-step resolution with full battle animations and companion override selector — `resolveNext` uses fire-and-forget `QueryOp`; `commitReplayOutcome` returns immediately and defers all DB writes to a second background `QueryOp`). The enemy sequence is **deterministic**: same pending queue → same enemies in every mode, regardless of team composition. Also hosts a **Battle History** tab (`history.html`) showing logs, companion levels, outcomes, and rewards for the last **500** resolved mobile battles. Communicates with `MobileBridge` to query statuses, handle batch dismissals, and deterministically run the battle resolution and reward calculations.
